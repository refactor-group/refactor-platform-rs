...
### Database
- `./scripts/rebuild_db.sh [db] [user] [schema]` — create/reset the local Postgres db, user, and schema (defaults: `refactor` / `refactor` / `refactor_platform`, password `password`)
- `cargo run --bin seed_db` — seed local test data
- `DATABASE_URL=postgres://refactor:password@localhost:5432/refactor sea-orm-cli migrate up -s refactor_platform` — run migrations manually against a running Postgres (database name must match whatever `rebuild_db.sh` created)
