...
| **PR 5 — Notes search via docs-collab-server** | Plain-text projection in/alongside the collab server's document store, ProseMirror-JSON→text flattener, persistence-hook ingestion, one-time Tiptap Cloud import at cutover, notes searcher (activates the `notes` type); measure `ts_rank` recompute cost on long documents — stored generated tsvector on the projection table if it hurts (see the FTS index decision) | M (~2–3 days) |
