...
| `coaching_relationship_id` | uuid, optional | Narrow to one coaching relationship ("everything within this engagement"). Applies to all relationship-anchored types; ignored for users/organizations. Cheap by construction: every searcher already anchors to a relationship id for scoping (see [Authorization scoping](#authorization-scoping)), so this is one more value tested against the same column. Intersect-only. |
